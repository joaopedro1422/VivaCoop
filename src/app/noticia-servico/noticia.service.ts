...
export class NoticiaService {
	...
	private news: News[] = [];
	private storageKey = 'viva_news'; // Chave para o localStorage

	constructor() {
		// Carrega as notícias do localStorage ao inicializar
		this.loadFromStorage();
	}

	private loadFromStorage(): void {
		const storedNews = localStorage.getItem(this.storageKey);
		if (storedNews) {
			this.news = JSON.parse(storedNews);
		} else {
			// Inicializa com dados padrão se o localStorage estiver vazio
			this.news = [
				{
					id: 1,
	...
	private saveToStorage(): void {
		localStorage.setItem(this.storageKey, JSON.stringify(this.news));
	}
	...
	addNews(news: News): Observable<News> {
		const newId = this.news.length > 0 ? Math.max(...this.news.map(n => n.id)) + 1 : 1;
